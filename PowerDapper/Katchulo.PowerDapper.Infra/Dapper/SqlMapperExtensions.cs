    ...
    public static class SqlMapperExtensions
    {
        ...
        public static T Get<T>(this IDbConnection connection, dynamic id, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            { 
                ...
                //Bruno Nascimento Ferreira, alteração para suportar ID customizado via mapeamento
                String idToUse = onlyKey.Name;
                var attrs = onlyKey.GetCustomAttributes(false).OfType<ColumnAttribute>();
                if (attrs.Count() > 0)
                {
                    idToUse = attrs.FirstOrDefault().Name;
                }
                idToUse = "[" + idToUse + "]";
                //
                sql = "select * from " + name + " where " + idToUse + " = @id";
        ...
        public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            using (var tx = transaction)
            {
                ...
                for (var i = 0; i < allProperties.Count(); i++)
                {
                    ...
                    //Alteração para permitir nomes customizados
                    var notMap = property.GetCustomAttributes(false).OfType<NotMappedAttribute>();
                    if (notMap.Count() > 0)
                    {
                        continue;
                    }

                    String nameToUse = property.Name;
                    var attrs = property.GetCustomAttributes(false).OfType<ColumnAttribute>();
                    if (attrs.Count() > 0)
                    {
                        nameToUse = attrs.FirstOrDefault().Name;
                    }
                    //
                    sb.Append("[" + nameToUse + "]");
                ...
                for (var i = 0; i < allProperties.Count(); i++)
                {
                    var property = allProperties.ElementAt(i);
                    if (keyProperties.Contains(property)) continue;
                    //Bruno Nascimento Ferreira, ignorando propriedade não mapeada
                    var notMap = property.GetCustomAttributes(false).OfType<NotMappedAttribute>();
                    if (notMap.Count() > 0)
                    {
                        continue;
        ...
        public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < nonIdProps.Count(); i++)
            {
                ...
                //
                sb.AppendFormat("{0} = @{1}", "[" + nameToUse + "]", property.Name);
                sb.AppendFormat(", ");
            }
            sb.Remove(sb.Length - 2, 2);
        ...
        public static bool Delete<T>(this IDbConnection connection, T entityToDelete, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
        {
            ...
            for (var i = 0; i < keyProperties.Count(); i++)
            {
                var property = keyProperties.ElementAt(i);
                //Bruno Nascimento Ferreira
                //Alteração para suportar anotação do nome da tabela
                String nameToUse = property.Name;
                var attrs = property.GetCustomAttributes(false).OfType<ColumnAttribute>();
                if (attrs.Count() > 0)
                {
